...
#include "freertos/FreeRTOS.h" // For FreeRTOS functions and macros
#include "freertos/task.h"     // For vTaskDelay and portTICK_PERIOD_MS
...
{
    ESP_LOGI(TAG_BME280, "Writing to register 0x%02X", reg_addr);

    esp_err_t err = i2c_write_custom(&bme280_interface, reg_addr, reg_data, len);
...
{
    ESP_LOGI(TAG_BME280, "Reading from register 0x%02X", reg_addr);
    ESP_LOGI(TAG_BME280, "Reading into: %p", reg_data);

    esp_err_t err = i2c_read_custom(&bme280_interface, reg_addr, reg_data, len);
    return (ESP_OK == err) ? 0 : -1; // Return 0 on success, -1 on error
}
...
esp_err_t bme280_init_driver(sensor_context_t *sensor_config)
{
    ...
    bme280.dev_addr = (u8)sensor_config->interface->config.i2c.config.device_address;
